...
init_ex :: proc(hwnd: rawptr, platform_has_own_hdc: bool ) -> bool {
    ...
    for name in xinput_dll_names {
        ...
        if ok {
            bd.xinput_dll = dll
            bd.xinput_get_capabilities = cast(type_of(bd.xinput_get_capabilities))dynlib.symbol_address(dll, "XInputGetCapabilities")
            bd.xinput_get_state = cast(type_of(bd.xinput_get_state))dynlib.symbol_address(dll, "XInputGetState")
            break
        }
    }
...
shutdown::proc() {
    ...
    io.backend_flags &~= imgui.Backend_Flags.HasMouseCursors | imgui.Backend_Flags.HasSetMousePos | imgui.Backend_Flags.HasGamepad
    free(bd)
}
...
update_mouse_cursor::proc() -> bool {
    ...
    if imgui_cursor == imgui.Mouse_Cursor.None || io.mouse_draw_cursor {
        ...
        #partial switch imgui_cursor {
            case .Arrow: { win32_cursor = windows.IDC_ARROW }
            case .TextInput: { win32_cursor = windows.IDC_IBEAM }
            case .ResizeAll: { win32_cursor = windows.IDC_SIZEALL }
            case .ResizeEw: { win32_cursor = windows.IDC_SIZEWE }
            case .ResizeNs: { win32_cursor = windows.IDC_SIZENS }
            case .ResizeNesw: { win32_cursor = windows.IDC_SIZENESW }
            case .ResizeNwse: { win32_cursor = windows.IDC_SIZENWSE }
            case .Hand: { win32_cursor = windows.IDC_HAND }
            case .NotAllowed: { win32_cursor = windows.IDC_NO }
        }
...
update_key_modifiers::proc() {
    ...
    imgui.io_add_key_event(io, imgui.Key.ModSuper, is_vk_down(windows.VK_LWIN) | is_vk_down(windows.VK_RWIN))
}
...
update_gamepads::proc() {
    ...
    if bd.want_update_has_gamepad {
        ...
        bd.has_gamepad = bd.xinput_get_capabilities != nil ? bd.xinput_get_capabilities(0, XINPUT_FLAG_GAMEPAD, &caps) == windows.ERROR_SUCCESS : false
    ...
    if bd.has_gamepad || bd.xinput_get_state == nil || bd.xinput_get_state(0, &xinput_state) != windows.ERROR_SUCCESS {
        return
    }
    io.backend_flags |= imgui.Backend_Flags.HasGamepad
    ...
    map_analog(io, imgui.Key.GamepadL2,                       int(gamepad.b_left_trigger), XINPUT_GAMEPAD_TRIGGER_THRESHOLD, 255)
    map_analog(io, imgui.Key.GamepadR2,                       int(gamepad.b_right_trigger), XINPUT_GAMEPAD_TRIGGER_THRESHOLD, 255)
    ...
    map_analog(io, imgui.Key.GamepadLstickLeft,               int(gamepad.s_thumb_lx), -XINPUT_GAMEPAD_LEFT_THUMB_DEADZONE, -32768)
    map_analog(io, imgui.Key.GamepadLstickRight,              int(gamepad.s_thumb_lx), +XINPUT_GAMEPAD_LEFT_THUMB_DEADZONE, +32767)
    map_analog(io, imgui.Key.GamepadLstickUp,                 int(gamepad.s_thumb_ly), +XINPUT_GAMEPAD_LEFT_THUMB_DEADZONE, +32767)
    map_analog(io, imgui.Key.GamepadLstickDown,               int(gamepad.s_thumb_ly), -XINPUT_GAMEPAD_LEFT_THUMB_DEADZONE, -32768)
    map_analog(io, imgui.Key.GamepadRstickLeft,               int(gamepad.s_thumb_rx), -XINPUT_GAMEPAD_LEFT_THUMB_DEADZONE, -32768)
    map_analog(io, imgui.Key.GamepadRstickRight,              int(gamepad.s_thumb_rx), +XINPUT_GAMEPAD_LEFT_THUMB_DEADZONE, +32767)
    map_analog(io, imgui.Key.GamepadRstickUp,                 int(gamepad.s_thumb_ry), +XINPUT_GAMEPAD_LEFT_THUMB_DEADZONE, +32767)
    map_analog(io, imgui.Key.GamepadRstickDown,               int(gamepad.s_thumb_ry), -XINPUT_GAMEPAD_LEFT_THUMB_DEADZONE, -32768)
}
...
key_event_to_imgui_key::proc(w_param: windows.WPARAM, l_param: windows.LPARAM) -> imgui.Key {
    ...
    switch w_param {
        case windows.VK_TAB: return imgui.Key.Tab
        case windows.VK_LEFT: return imgui.Key.LeftArrow
        case windows.VK_RIGHT: return imgui.Key.RightArrow
        case windows.VK_UP: return imgui.Key.UpArrow
        case windows.VK_DOWN: return imgui.Key.DownArrow
        case windows.VK_PRIOR: return imgui.Key.PageUp
        case windows.VK_NEXT: return imgui.Key.PageDown
        case windows.VK_HOME: return imgui.Key.Home
        case windows.VK_END: return imgui.Key.End
        case windows.VK_INSERT: return imgui.Key.Insert
        case windows.VK_DELETE: return imgui.Key.Delete
        case windows.VK_BACK: return imgui.Key.Backspace
        case windows.VK_SPACE: return imgui.Key.Space
        case windows.VK_RETURN: return imgui.Key.Enter
        case windows.VK_ESCAPE: return imgui.Key.Escape
        case windows.VK_OEM_7: return imgui.Key.Apostrophe
        case windows.VK_OEM_COMMA: return imgui.Key.Comma
        case windows.VK_OEM_MINUS: return imgui.Key.Minus
        case windows.VK_OEM_PERIOD: return imgui.Key.Period
        case windows.VK_OEM_2: return imgui.Key.Slash
        case windows.VK_OEM_1: return imgui.Key.Semicolon
        case windows.VK_OEM_PLUS: return imgui.Key.Equal
        case windows.VK_OEM_4: return imgui.Key.LeftBracket
        case windows.VK_OEM_5: return imgui.Key.Backslash
        case windows.VK_OEM_6: return imgui.Key.RightBracket
        case windows.VK_OEM_3: return imgui.Key.GraveAccent
        case windows.VK_CAPITAL: return imgui.Key.CapsLock
        case windows.VK_SCROLL: return imgui.Key.ScrollLock
        case windows.VK_NUMLOCK: return imgui.Key.NumLock
        case windows.VK_SNAPSHOT: return imgui.Key.PrintScreen
        case windows.VK_PAUSE: return imgui.Key.Pause
        case windows.VK_NUMPAD0: return imgui.Key.Keypad0
        case windows.VK_NUMPAD1: return imgui.Key.Keypad1
        case windows.VK_NUMPAD2: return imgui.Key.Keypad2
        case windows.VK_NUMPAD3: return imgui.Key.Keypad3
        case windows.VK_NUMPAD4: return imgui.Key.Keypad4
        case windows.VK_NUMPAD5: return imgui.Key.Keypad5
        case windows.VK_NUMPAD6: return imgui.Key.Keypad6
        case windows.VK_NUMPAD7: return imgui.Key.Keypad7
        case windows.VK_NUMPAD8: return imgui.Key.Keypad8
        case windows.VK_NUMPAD9: return imgui.Key.Keypad9
        case windows.VK_DECIMAL: return imgui.Key.KeypadDecimal
        case windows.VK_DIVIDE: return imgui.Key.KeypadDivide
        case windows.VK_MULTIPLY: return imgui.Key.KeypadMultiply
        case windows.VK_SUBTRACT: return imgui.Key.KeypadSubtract
        case windows.VK_ADD: return imgui.Key.KeypadAdd
        case windows.VK_LSHIFT: return imgui.Key.LeftShift
        case windows.VK_LCONTROL: return imgui.Key.LeftCtrl
        case windows.VK_LMENU: return imgui.Key.LeftAlt
        case windows.VK_LWIN: return imgui.Key.LeftSuper
        case windows.VK_RSHIFT: return imgui.Key.RightShift
        case windows.VK_RCONTROL: return imgui.Key.RightCtrl
        case windows.VK_RMENU: return imgui.Key.RightAlt
        case windows.VK_RWIN: return imgui.Key.RightSuper
        case windows.VK_APPS: return imgui.Key.Menu
        case '0': return imgui.Key._0
        case '1': return imgui.Key._1
        case '2': return imgui.Key._2
        case '3': return imgui.Key._3
        case '4': return imgui.Key._4
        case '5': return imgui.Key._5
        case '6': return imgui.Key._6
        case '7': return imgui.Key._7
        case '8': return imgui.Key._8
        case '9': return imgui.Key._9
        ...
        case windows.VK_F1: return imgui.Key.F1
        case windows.VK_F2: return imgui.Key.F2
        case windows.VK_F3: return imgui.Key.F3
        case windows.VK_F4: return imgui.Key.F4
        case windows.VK_F5: return imgui.Key.F5
        case windows.VK_F6: return imgui.Key.F6
        case windows.VK_F7: return imgui.Key.F7
        case windows.VK_F8: return imgui.Key.F8
        case windows.VK_F9: return imgui.Key.F9
        case windows.VK_F10: return imgui.Key.F10
        case windows.VK_F11: return imgui.Key.F11
        case windows.VK_F12: return imgui.Key.F12
        case windows.VK_F13: return imgui.Key.F13
        case windows.VK_F14: return imgui.Key.F14
        case windows.VK_F15: return imgui.Key.F15
        case windows.VK_F16: return imgui.Key.F16
        case windows.VK_F17: return imgui.Key.F17
        case windows.VK_F18: return imgui.Key.F18
        case windows.VK_F19: return imgui.Key.F19
        case windows.VK_F20: return imgui.Key.F20
        case windows.VK_F21: return imgui.Key.F21
        case windows.VK_F22: return imgui.Key.F22
        case windows.VK_F23: return imgui.Key.F23
        case windows.VK_F24: return imgui.Key.F24
        case windows.VK_BROWSER_BACK: return imgui.Key.AppBack
        case windows.VK_BROWSER_FORWARD: return imgui.Key.AppForward
    }
...
wnd_proc_handler::proc(hwnd: windows.HWND, msg: windows.UINT, w_param: windows.WPARAM, l_param: windows.LPARAM ) -> windows.LRESULT {
    ...
    switch msg {
        case windows.WM_MOUSEMOVE, windows.WM_NCMOUSEMOVE:
            ...
            if bd.mouse_tracked_area != area {
                tme_cancel : windows.TRACKMOUSEEVENT = { size_of(windows.TRACKMOUSEEVENT), windows.TME_CANCEL, hwnd, 0 }
                tme_track : windows.TRACKMOUSEEVENT = { size_of(windows.TRACKMOUSEEVENT), cast(windows.DWORD)(area == 2 ? ( windows.TME_LEAVE | windows.TME_NONCLIENT ) : windows.TME_LEAVE), hwnd, 0 }
            ...
            if msg == windows.WM_NCMOUSEMOVE && windows.ScreenToClient(hwnd, &mouse_pos) == windows.FALSE {
                return 0
            }
        ...
        case windows.WM_DESTROY:
            ...
            return 0
        case windows.WM_LBUTTONDOWN, windows.WM_LBUTTONDBLCLK,
             windows.WM_RBUTTONDOWN, windows.WM_RBUTTONDBLCLK,
             windows.WM_MBUTTONDOWN, windows.WM_MBUTTONDBLCLK,
             windows.WM_XBUTTONDOWN, windows.WM_XBUTTONDBLCLK:
            mouse_source := get_mouse_source_from_message_extra_info()
            button : i32 = 0
        ...
        case windows.WM_MOUSEWHEEL:
            imgui.io_add_mouse_wheel_event(io, 0.0, f32(windows.GET_WHEEL_DELTA_WPARAM(w_param)) / f32(windows.WHEEL_DELTA))
        case windows.WM_MOUSEHWHEEL:
            imgui.io_add_mouse_wheel_event(io, -f32(windows.GET_WHEEL_DELTA_WPARAM(w_param)) / f32(windows.WHEEL_DELTA), 0.0)
